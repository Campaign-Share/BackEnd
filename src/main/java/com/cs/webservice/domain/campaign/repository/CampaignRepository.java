...
public interface CampaignRepository extends JpaRepository<Campaign, String> {
    ...
    @Query(value = "SELECT * FROM campaigns WHERE uuid IN (SELECT DISTINCT campaign_uuid FROM campaign_tags WHERE tag=?1) " +
            "AND status=?2 AND CURRENT_DATE() <= CAST(end_date AS DATETIME) ORDER BY agree_number + disagree_number DESC LIMIT ?4 OFFSET ?3", nativeQuery = true)
    List<Campaign> findAllByTagAndStatusWithPagingSortedByFamous(String tag, Integer status, Integer start, Integer count);
    @Query(value = "SELECT * FROM campaigns WHERE status=?1 AND CURRENT_DATE() <= CAST(end_date AS DATETIME) ORDER BY agree_number + disagree_number DESC LIMIT ?3 OFFSET ?2", nativeQuery = true)
    List<Campaign> findAllByStatusWithPagingSortedByFamous(Integer status, Integer start, Integer count);
    @Query(value = "SELECT * FROM campaigns WHERE uuid IN (SELECT DISTINCT campaign_uuid FROM campaign_tags WHERE tag=?1) " +
            "AND CURRENT_DATE() <= CAST(end_date AS DATETIME) ORDER BY agree_number + disagree_number DESC LIMIT ?3 OFFSET ?2", nativeQuery = true)
    List<Campaign> findAllByTagWithPagingSortedByFamous(String tag, Integer start, Integer count);
    @Query(value = "SELECT * FROM campaigns WHERE CURRENT_DATE() <= CAST(end_date AS DATETIME) ORDER BY agree_number + disagree_number DESC LIMIT ?2 OFFSET ?1", nativeQuery = true)
    List<Campaign> findAllWithPagingSortedByFamous(Integer start, Integer count);

    @Query(value = "SELECT * FROM campaigns WHERE uuid IN (SELECT DISTINCT campaign_uuid FROM campaign_tags WHERE tag=?1) " +
            "AND status=?2 AND CURRENT_DATE() <= CAST(end_date AS DATETIME) ORDER BY RAND() DESC LIMIT ?4 OFFSET ?3", nativeQuery = true)
    List<Campaign> findAllByTagAndStatusWithPagingSortedByRandom(String tag, Integer status, Integer start, Integer count);
    @Query(value = "SELECT * FROM campaigns WHERE status=?1 AND CURRENT_DATE() <= CAST(end_date AS DATETIME) ORDER BY RAND() DESC LIMIT ?3 OFFSET ?2", nativeQuery = true)
    List<Campaign> findAllByStatusWithPagingSortedByRandom(Integer status, Integer start, Integer count);
    @Query(value = "SELECT * FROM campaigns WHERE uuid IN (SELECT DISTINCT campaign_uuid FROM campaign_tags WHERE tag=?1) " +
            "AND CURRENT_DATE() <= CAST(end_date AS DATETIME) ORDER BY RAND() DESC LIMIT ?3 OFFSET ?2", nativeQuery = true)
